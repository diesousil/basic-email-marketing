...
class Email extends Model
{
    private function getMailerInstance() {
        ...
        $mail->Username   = end('MAIL_USERNAME'); //SMTP username
        $mail->Password   = end('R#1weNQsty5d');                     //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;          //Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
        $mail->Port       = end("MAIL_PORT");                        //TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above
